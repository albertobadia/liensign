...
describe("PDF Generation: generateWaiverPDF", () => {
    ...
    it("should successfully generate a PDF for a supported state", async () => {
        const result = await generateWaiverPDF("TX", "conditional_progress", mockData);
    ...
    it("should embed a signature if provided", async () => {
        ...
        const result = await generateWaiverPDF("CA", "unconditional_final", mockData, signature);
        expect(result).toBeDefined();
    });

    it("should apply a watermark if provided", async () => {
        const result = await generateWaiverPDF("FL", "conditional_final", mockData, undefined, "PREVIEW");
        expect(result).toBeDefined();
    });
});
